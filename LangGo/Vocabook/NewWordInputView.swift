...
struct NewWordInputView: View {
    ...
    private func handleResultSelection(_ result: SearchResult) {
        if inputDirection == .baseToTarget {
            self.word = result.word; self.baseText = result.definition
        } else {
            self.word = result.definition; self.baseText = result.word
        }
    ...
    private func debouncedSearch(term: String, searchBase: Bool) {
        ...
        searchTask = Task {
            do {
                ...
                // CORRECTED: This now checks for both kinds of cancellation errors.
                if error is CancellationError || (error as? URLError)?.code == .cancelled {
                    // This is an expected cancellation, so we handle it silently.
                    await MainActor.run {
                        self.isSearching = false
                    }
                } else {
                    // This is a different, unexpected error. Show it to the user.
                    await MainActor.run {
...
struct SearchResult: Identifiable, Hashable {
    let id: String; let word: String; let definition: String; let partOfSpeech: String
